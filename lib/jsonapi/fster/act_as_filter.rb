          ...
          params.each do |key, value|
            ...
            if column_names.include? key
              ...
              if belongs_to_relationship? key
                ...
                # course
                query = query.public_send(joins_clause, key.to_sym).where(key.pluralize => {id: value})
              elsif has_many_relationship? key
                # courses
                query = query.public_send(joins_clause, key.to_sym).where(key => {id: value})
              else
                attrs = key.split('.')
                model1 = attrs[0]
                attr = attrs.last
                ...
                if attrs.size == 2
                  if model1_class.column_names.include? attr
                    # course.name
                    query = query.public_send(joins_clause, model1.to_sym).where(model1.pluralize => {attr => value})
                  elsif model1_class.belongs_to_relationship? attr
                    # course.amenity
                    query = query.public_send(joins_clause, model1.to_sym).where(model1.pluralize => {"#{attr}_id" => value})
                  elsif model1_class.has_one_relationship? attr
                    # course.amenity
                    query = query.public_send(joins_clause, model1.to_sym => attr.to_sym).where(attr.pluralize => {id: value})
                  elsif model1_class.has_many_relationship? attr
                    # course.amenities
                    query = query.public_send(joins_clause, model1.to_sym => attr.to_sym).where(attr => {id: value})
                    ...
                    query = query.public_send(joins_clause, model1.to_sym => through_table).where(through_table => {"#{attr.singularize}_id": value})
                  end
                elsif attrs.size == 3
                  model2 = attrs[1]
                  query = query.public_send(joins_clause, model1.to_sym => model2.to_sym)
                  if model1_class.belongs_to_relationship?(model2) || model1_class.has_one_relationship?(model2)
                    # course.amenity.name = 'ab'
                    query = query.public_send(joins_clause, model1.to_sym => model2.to_sym).where(model2.pluralize => {attr => value})
